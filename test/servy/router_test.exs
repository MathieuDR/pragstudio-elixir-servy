defmodule Servy.RouterTest do
  ...
  describe "route/1" do
    test "GET /wildthings routes correctly" do
      ...
      assert %{resp_body: "Bears, Lions, Tigers", status: 200} = Router.route(conv)
    end

    test "GET /bears routes correctly" do
      conv = ServySupportTest.create_conv("GET", "/bears")
      assert %{resp_body: "Teddy, Smokey, Paddington", status: 200} = Router.route(conv)
    end

    test "GET /bears/5 routes correctly" do
      conv = ServySupportTest.create_conv("GET", "/bears/5")
      assert %{resp_body: "Bear 5", status: 200} = Router.route(conv)
    end

    test "DELETE /bears routes correctly" do
      conv = ServySupportTest.create_conv("DELETE", "/bears/5")
      assert %{resp_body: "Deleted bear 5", status: 200} = Router.route(conv)
    end
    ...
    test "GET /pages/bla.md returns 404" do
      ...
      assert %{resp_body: "File not found!", status: 404} = Router.route(conv)
    end
    ...
    test "GET /pages/bla.php returns 500" do
      ...
      assert %{resp_body: "Something went wrong: wrong_ext", status: 500} = Router.route(conv)
    end
  end
end
